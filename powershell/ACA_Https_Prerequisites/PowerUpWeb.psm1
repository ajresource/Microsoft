...
function enable-aspnetisapi($isapiPath){
	if (!(Test-Path $isapiPath)) {
		write-host "$isapiPath doesn't exist and is required"
	}
...
function enable-aspnet()
{
	...
	if (!(Test-Path $regiis)) {
		write-host "$regiis does not exist"
	}
	...
	if ($lastexitcode -ne 0)
	{
		write-host "Unable to register asp.net with IIS as lastexitcode is $lastexitcode"
		write-host $output
